...
// ── 尺寸格式：160*105*72 cm ───────────────────────────────────────────────────
const DIM_RE = /^(\d+[.,]?\d*)\s*\*\s*(\d+[.,]?\d*)\s*\*\s*(\d+[.,]?\d*)\s*(cm)?$/i
...
function calcVolume(dims: string[]): number {
  ...
  for (const d of dims) {
    ...
    if (m) total += parseFloat(m[1]) * parseFloat(m[2]) * parseFloat(m[3]) / 1_000_000
  }
...
function extractAllDimensions(d14: string): string[] {
  ...
  while ((m = pattern.exec(d14)) !== null) {
    ...
    dims.push(`${n(m[1])}*${n(m[2])}*${n(m[3])} cm`)
  }
  return dims
}
...
export default function NewInquiryPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (street)  body.address = street
      if (remark)  body.remark  = remark

      const { pageId } = await apiFetch<{ pageId: string }>('/api/records', {
        method: 'POST',
        body: JSON.stringify(body),
      })
      navigate(`/detail/${pageId}`, { replace: true })
    } catch (e) {
      setError(String(e))
      setSubmitting(false)
    }
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-medium text-gray-600">
                货物尺寸 (cm) *
                <span className="ml-1 text-gray-400 font-normal">格式：160*105*72 cm</span>
              </label>
              {volume != null && (
                ...
                <div key={i} className="flex items-center gap-2">
                  ...
                  <div className="flex-1">
                    <input
                      ...
                      placeholder="如：160*105*72 cm"
                      className={`${inputCls} font-mono ${dimErrors[i] ? 'border-red-400 ring-1 ring-red-400' : ''}`}
                    />
                    {dimErrors[i] && (
                      <p className="text-xs text-red-500 mt-0.5">格式错误，请输入 L*W*H cm（如：160*105*72 cm）</p>
                    )}
                  </div>
                </div>
              ))}
